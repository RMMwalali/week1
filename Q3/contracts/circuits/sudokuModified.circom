...
template sudoku() {
    ...
    component RangeProofPuzzle[82];
    component RangeProofSolution[82];
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            var count = i * 9 + j;

            RangeProofSolution[count] = RangeProof(32);
            RangeProofSolution[count].in <== puzzle[i][j];
            RangeProofSolution[count].range[0] <== 0;
            RangeProofSolution[count].range[1] <== 9;
            RangeProofSolution[count].out === 1;

            RangeProofPuzzle[count] = RangeProof(32);
            RangeProofPuzzle[count].in <== puzzle[i][j];
            RangeProofPuzzle[count].range[0] <== 0;
            RangeProofPuzzle[count].range[1] <== 9;
            RangeProofPuzzle[count].out === 1;
